...
final class Performance extends Module {
  ...
  Future<(T, Duration)> timed<T>(
    ...
    while (!done) {
      if (sw.elapsed > deadline) {
        fail('$label did not complete within ${deadline.inMilliseconds}ms — '
  ...
  Future<void> videoLatency() async {
    ...
    expect(startElapsed, lessThan(const Duration(seconds: 4)),
  ...
  Future<void> backToBackCapture() async {
    ...
    await timed(() => _ctrl.startRecording(path),
        deadline: const Duration(seconds: 10), label: 'b2b video start');
    await pumpFor(const Duration(seconds: 1));
    final (rec, _) = await timed(() => _ctrl.stopRecording(),
        deadline: const Duration(seconds: 12), label: 'b2b video stop');
  ...
  Future<void> deviceEnumWarm({
    ...
    for (var i = 1; i <= 3; i++) {
      ...
      expect(sw.elapsed, lessThan(budget),
          reason: 'warm enumeration #$i took ${sw.elapsedMilliseconds}ms '
  ...
  Future<void> setterLatency({
    ...
    for (final entry in setters.entries) {
      ...
      expect(best, lessThan(budget),
          reason: '${entry.key} blocked for ${best.inMilliseconds}ms — '
    ...
    final sw = Stopwatch()..start();
    await c.configure(base.copyWith(
      zoom: 1.0,
      exposure: 0.0,
      flash: FlashMode.off,
