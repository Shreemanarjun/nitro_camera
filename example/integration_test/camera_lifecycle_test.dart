...
void main() {
  ...
  testWidgets(
    '1. app boots to a running camera preview',
    ...
    (tester) async {
      ...
      expect(
        cameraStore.activeTextureId.value,
        isNot(0),
  ...
  testWidgets(
    '2. survives rapid device switching (6 toggles, 1s gaps)',
    semanticsEnabled: false,
    (tester) async {
      await bootApp(tester);
  ...
  testWidgets(
    '3. custom frame processor receives frames and survives a '
    'SCANNER round-trip',
    semanticsEnabled: false,
    (tester) async {
      await bootApp(tester);
      ...
      await pumpUntil(
        ...
        timeout: const Duration(seconds: 15),
        reason:
            'frames flowed through the custom processor '
      ...
      if (cameraStore.devices.value.length > 1) {
        ...
        await pumpUntil(
          ...
          timeout: const Duration(seconds: 15),
          reason:
              'preview running on the new device with the processor installed',
        );
        await pumpUntil(
          tester,
          ...
          timeout: const Duration(seconds: 15),
          reason:
              'frames keep flowing through the processor after a lens change',
        );
        expect(
          luminanceProcessor.attached.value,
          isTrue,
  ...
  testWidgets(
    '4. resolution change 1080p -> 720p -> 1080p survives',
    semanticsEnabled: false,
    (tester) async {
      await bootApp(tester);
