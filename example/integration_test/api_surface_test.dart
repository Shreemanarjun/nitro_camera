...
void main() {
  ...
  testWidgets('1. permissions: status queries agree with the request path',
  ...
  testWidgets('2. typed device enumeration, selectors and format resolver',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    ...
    final wide = selectCameraDevice(devices,
        position: CameraPosition.back,
        physicalDevices: [PhysicalDeviceType.wideAngleCamera]);
    if (back != null) {
      expect(wide, isNotNull,
          reason: 'wide-angle back selector must resolve when a back camera exists');
    }
  ...
  testWidgets('3. session state + declarative configure() without a reopen',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets('4. every imperative live setter leaves a healthy session',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets('5. torch + torch level (skipped without a torch unit)',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets('6. preview pause/resume via setActive keeps the session',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets(
      ...
      'zero-copy contract', semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
    ...
    for (final f in frames) {
      ...
      if (f.rowBytes > 0) {
        expect(f.size, f.rowBytes * f.h,
            reason: 'size must equal stride × height');
      }
    }
  });

  testWidgets('8. frame-processor plugin registry runs on a worker isolate',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets(
    ...
    expect(rec.durationMs, lessThan(2600),
  ...
  testWidgets('10. photo option variants: geotag, silent+speed, DNG',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
  ...
  testWidgets('11. native detector smoke: barcode on/off keeps frames flowing',
      semanticsEnabled: false, (tester) async {
    await bootApp(tester);
    final c = ctrl();
    ...
    expect(cameraStore.status.value, CameraStatus.running);
    expect(cameraStore.errorMessage.value, isNull,
        reason: 'detector on/off must not surface an error '
