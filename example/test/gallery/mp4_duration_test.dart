...
void main() {
  ...
  test('reads duration from mvhd v1', () async {
    final bytes = BytesBuilder()
      ..add(box('ftyp', 'isom0000'.codeUnits))
      ..add(box('moov', box('mvhd', mvhdV1(timescale: 1000, duration: 12345))));
    final path = await writeTemp(bytes.toBytes());
    expect(await probeMp4Duration(path), const Duration(milliseconds: 12345));
  });
  ...
  test('finds moov after a large mdat (non-faststart layout)', () async {
    final bytes = BytesBuilder()
      ...
      ..add(
        box('moov', box('mvhd', mvhdV0(timescale: 90000, duration: 90000))),
      );
    final path = await writeTemp(bytes.toBytes());
  ...
  test('returns null for the v0 unknown-duration sentinel', () async {
    final bytes = BytesBuilder()
      ..add(
        box('moov', box('mvhd', mvhdV0(timescale: 600, duration: 0xFFFFFFFF))),
      );
    final path = await writeTemp(bytes.toBytes());
  ...
  test('returns null for garbage / truncated files', () async {
    ...
    expect(
      await probeMp4Duration(await writeTemp(List.filled(64, 0xAB))),
      isNull,
    );
