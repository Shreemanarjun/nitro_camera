...
void main() {
  ...
  group('CaptureStorage.persist', () {
    test(
      'moves a JPEG into captures/ as IMG_<timestamp>.jpg, bytes intact',
      () async {
    ...
    test('names DNG captures RAW_ and videos VID_', () async {
      ...
      final dng = await storage.persist(
        cacheFile('cap_1.dng', [1]).path,
        capturedAt: when,
      );
      final mp4 = await storage.persist(
        cacheFile('video_1.mp4', [2]).path,
        capturedAt: when,
      );
    ...
    test('same-second captures get a collision suffix', () async {
      final when = DateTime(2026, 7, 3, 15, 42, 10);
      final a = await storage.persist(
        cacheFile('a.jpg', [1]).path,
        capturedAt: when,
      );
      final b = await storage.persist(
        cacheFile('b.jpg', [2]).path,
        capturedAt: when,
      );
  ...
  group('CaptureStorage.loadAll', () {
    test(
      'returns library entries newest first across media prefixes',
      () async {
        ...
        await storage.persist(
          cacheFile('b.mp4', [2]).path,
          capturedAt: DateTime(2026, 7, 2, 10, 0, 0),
        );
        await storage.persist(
          cacheFile('c.dng', [3]).path,
    ...
    test('ignores the .thumbs cache dir and unknown files', () async {
      ...
      File(
        '${(await storage.capturesDir()).path}/notes.txt',
