...
void main() {
  test('factory codes are stable', () {
    ...
    expect(PermissionException.microphoneDenied().code, 'permission/microphone-denied');
    ...
    expect(SessionException.malformedPayload('x', 'oops').code, 'session/malformed-payload');
  ...
  test('messages carry the human-readable detail', () {
    ...
    expect(SessionException.nativeError('').message, isNotEmpty, reason: 'an empty native message must not produce an empty error');
  ...
  test('the hierarchy is exhaustively matchable (sealed)', () {
    ...
    expect(domain(const RecorderException('recorder/writer-failed', 'w')), 'recorder');
  });
