...
CameraDeviceInfo dev({String id = 'back'}) => CameraDeviceInfo(
  id: id,
  name: 'Camera $id',
...
class GatedFake extends FakeNitroCamera {
  /// Set to hold the NEXT `openCamera` until the completer is completed.
...
/// Unblocks a session restart, then renders the result.
///
/// `_restart` opens with `await _eventSub.cancel()`. A `StreamSubscription`'s
/// cancel future is bound to the ROOT zone, so `FakeAsync` (which every
/// `testWidgets` body runs inside) never schedules its continuation — pumping
/// alone leaves the restart parked on line one. `runAsync` turns the real event
/// loop once, which drains it; everything after that await is fake-clock again.
Future<void> drainRestart(WidgetTester tester) async {
...
void main() {
  ...
  group('CameraView initial open', () {
    ...
    testWidgets('the child overlay renders on top of the preview', (tester) async {
      await tester.pumpWidget(wrap(CameraView(device: dev(), native: fake, child: const Text('hud'))));
      await tester.pumpAndSettle();
    ...
    testWidgets('without a format no resolved config is reported', (tester) async {
      ResolvedCameraConfig? resolved;

      await tester.pumpWidget(
        wrap(CameraView(device: dev(), native: fake, onConfigResolved: (r) => resolved = r)),
      );
      await tester.pumpAndSettle();
  ...
  group('CameraView open failure', () {
    testWidgets('retries with exponential backoff, gives up after 5 attempts', (tester) async {
      ...
      expect(fake.countOf('openCamera'), 2, reason: 'the backoff doubled to 500ms');
      await tester.pump(const Duration(milliseconds: 1));
    ...
    testWidgets('without an errorBuilder the loading state stays and onError still fires', (tester) async {
      ...
      for (final ms in [250, 500, 1000, 2000]) {
  ...
  group('CameraView didUpdateWidget', () {
    ...
    testWidgets('a device switch double-buffers: the retiring preview outlives the new open', (tester) async {
      ...
      expect(
        fake.callNames.indexOf('closeCamera') < fake.callNames.lastIndexOf('openCamera'),
        isTrue,
      ...
      expect(fake.countOf('closeCamera'), 1, reason: 'closeSession already freed the hardware');
    ...
    testWidgets('a device switch that reuses the texture id renders a single preview', (tester) async {
      ...
      await tester.pumpWidget(view(settleDelay: settle));
      await tester.pumpAndSettle();

      await tester.pumpWidget(view(id: 'front', settleDelay: settle));
      await drainRestart(tester);

      expect(fake.countOf('openCamera'), 2, reason: 'the new session really did open');
      expect(find.byType(CameraPreview), findsOneWidget, reason: 'the duplicate texture id is not double-mounted');
      expect(find.byKey(const ValueKey('nitra_preview_7')), findsOneWidget);
      expect(find.byKey(const ValueKey('nitra_retiring_7')), findsNothing);
      expect(tester.takeException(), isNull);
    ...
    testWidgets('a resolution change tears down first, waits out settleDelay, then reopens', (tester) async {
      ...
      expect(find.byType(CircularProgressIndicator), findsOneWidget);
      expect(find.byKey(const ValueKey('nitra_retiring_7')), findsNothing, reason: 'no double buffering here');
  ...
  group('CameraView session events', () {
    ...
    testWidgets('the event subscription follows the reopened session', (tester) async {
      final events = <CameraSessionEvent>[];
      Widget view(int width) =>
          wrap(CameraView(device: dev(), width: width, height: 720, native: fake, onEvent: events.add));
  ...
  group('CameraView teardown', () {
    testWidgets('unmounting mid-open disposes the controller and never setStates', (tester) async {
      ...
      await tester.pumpWidget(wrap(CameraView(device: dev(), native: fake, onClosing: () => closings++)));
      expect(find.byType(CircularProgressIndicator), findsOneWidget);
    ...
    testWidgets('a restart queued behind an unmount aborts instead of reopening', (tester) async {
      ...
      // new controller already published, so the queue stays busy.
      fake.openCameraResult = 9;
      ...
      await tester.pumpWidget(wrap(const SizedBox()));
      expect(closings, 2, reason: 'dispose reports the teardown');

      await tester.pump(settle);
      await drainRestart(tester);
      await tester.pumpAndSettle();

      expect(closings, 3, reason: 'the queued restart still announces its teardown');
      expect(fake.countOf('openCamera'), 2, reason: 'the queued restart must not open a third session');
      expect(fake.countOf('closeCamera'), 2);
      expect(tester.takeException(), isNull);
    });
  });
}
